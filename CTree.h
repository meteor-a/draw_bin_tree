...
class CTree
{
    public:
        ...
        int del(const T* const pKey);
        CNode<T>* getFirst() { return m_pRoot; }
		int removeAll(void);
        CNode<T>* getMinNode(CNode<T>* pNode) const;
        CNode<T>* getMaxNode(CNode<T>* pNode) const;

        long long HeightOfTree() {
...
template<class T>
long long CTree<T>::getHeight(CNode<T>* node) {
    if (node == NULL)
        return 0;
    long long left;
    if (node->getChild(leftChild) != NULL) {
        left = getHeight(node->getChild(leftChild));
    }
    else
        left = -1;
    return left + 1;
 ...
 */
template<class T> 
int CTree<T>::del(const T* const pKey)
{
    CNode<T>* pNode = 0;
